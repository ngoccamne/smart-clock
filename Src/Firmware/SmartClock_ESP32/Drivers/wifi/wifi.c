...
static void event_wifi_sta_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
...
void wifi_init_sta(void)
{
    ...
    esp_event_handler_instance_register(WIFI_EVENT, ESP_EVENT_ANY_ID, &event_wifi_sta_handler, NULL, &instance_any_id);
    esp_event_handler_instance_register(IP_EVENT, IP_EVENT_STA_GOT_IP, &event_wifi_sta_handler, NULL, &instance_got_ip);
    ESP_LOGI(TAG_1, "Error check done");

    wifi_config_t wifi_config = {.sta = {.ssid               = ESP_WIFI_SSID,
    ...
    {
        ESP_LOGI(TAG_1, "Connected to ap SSID:%s password:%s", ESP_WIFI_SSID, ESP_WIFI_PASS);
    }
    else if (bits & WIFI_FAIL_BIT)
    {
        ESP_LOGI(TAG_1, "Failed to connect to SSID:%s, password:%s", ESP_WIFI_SSID, ESP_WIFI_PASS);
    }
    else
    {
